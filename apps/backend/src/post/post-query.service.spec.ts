...
describe("PostQueryService", () => {
  ...
  describe("existsPost", () => {
    it("success: 글이 존재하는 경우 true 반환", async () => {
      // Given: 글이 존재하는 경우
      const user = await saveTestUser();
      const post = await saveTestPost(user.id);

      // When: 존재 여부 확인 시도
      const result = await postQueryService.existsPost(post.id);

      // Then: true 반환
      expect(result).toBe(true);
    });

    it("success: 글이 존재하지 않는 경우 false 반환", async () => {
      // Given: 글이 존재하지 않는 경우
      const nonExistentId = 0;

      // When: 존재 여부 확인 시도
      const result = await postQueryService.existsPost(nonExistentId);

      // Then: false 반환
      expect(result).toBe(false);
    });

    it("success: 삭제된 글도 존재하는 것으로 처리", async () => {
      // Given: soft-delete된 글 (getPost와 달리 activity 여부를 확인하지 않음)
      const author = await saveTestUser();
      const post = await saveTestPost(author.id);
      await userRepository.softDelete({ id: author.id });
